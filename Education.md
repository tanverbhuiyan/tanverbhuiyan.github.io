---
layout: default
title: Education
permalink: /education/
---

# 📚 Formal Education  

- **Bachelor of Science** | **March 2022 – Present**
  **Department of Statistics**  
  *Comilla University, Bangladesh*  

  - **Relevant Courses to AI/ML & Data Science:**  
    Data     

- **Higher Secondary Certificate (HSC)** | **2017 – 2019**  
  Science Group  
  *xyz*    

---

# ⚙️ Training Programs & Internships  

- **Aspire Leaders Program 2023** – *Aspire Institute, Harvard University*  
  ▪ Training on leadership, strategic thinking, and social impact.  

- **Data Analytics Intern** – *xyz*  
  ▪ Learned data visualization and reporting using Excel and Power BI.  


---

# 🧾 Online Courses & Certifications  

- **Coursera – DeepLearning.ai**  

- **Google Career Certificates**  
  - Data Analytics, Advanced Data Analytics, Business Intelligence  
  - IT Automation with Python, IT Support, Project Management  

- **DataCamp**  
  - Data Scientist with Python  
  - Machine Learning Scientist with Python  

- **Other Platforms**  
  - FreeCodeCamp – Data Analysis with Python  
  - HarvardX (edX) – Using Python for Research  
  - IBM Cognitive Class – Applied Data Science with Python  
  - Sololearn – Python, Data Science, Machine Learning  






